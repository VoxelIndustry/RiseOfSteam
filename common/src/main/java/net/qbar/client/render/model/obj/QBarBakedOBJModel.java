...
public class QBarBakedOBJModel extends OBJModel.OBJBakedModel
{
                             ...
                             final ImmutableMap<String, TextureAtlasSprite> textures)
    {
        ...
        this.cachedVariants = CacheBuilder.newBuilder().weakKeys().expireAfterAccess(20, TimeUnit.MINUTES)
                .build(new CacheLoader<QBarOBJState, ImmutableList<BakedQuad>>()
                {
                    @Override
    ...
    public List<BakedQuad> getQuads(IBlockState blockState, EnumFacing side, long rand)
    {
        ...
        if (blockState instanceof IExtendedBlockState)
        {
            ...
            if (exState.getUnlistedNames().contains(QBarStateProperties.VISIBILITY_PROPERTY))
            {
                QBarOBJState newState = exState.getValue(QBarStateProperties.VISIBILITY_PROPERTY);
                if (newState != null)
                {
                    try
                    {
    ...
    private ImmutableList<BakedQuad> buildQuads(IModelState modelState)
    {
        ...
        for (OBJModel.Group g : this.model.getMatLib().getGroups().values())
        {
            ...
            if (modelState instanceof QBarOBJState)
            {
                QBarOBJState state = (QBarOBJState) modelState;
