...
public class QBarMachines
{
    ...
    static
    {
     /*   OFFSHORE_PUMP = new MachineDescriptor("offshorepump", EMachineTier.TIER1, EMachineType.LOGISTIC)
                .component(SteamComponent.builder().steamCapacity(2000).steamConsumption(5)
                        .workingPressure(SteamUtil.BASE_PRESSURE).maxPressureCapacity(1.5f * SteamUtil.BASE_PRESSURE)
                        .allowOvercharge(true).build());

        SMALL_MINING_DRILL = new MachineDescriptor("smallminingdrill", EMachineTier.TIER1, EMachineType.RESOURCE_PRODUCER)
                .component(SteamComponent.builder().steamCapacity(2000).steamConsumption(25)
                        .workingPressure(SteamUtil.BASE_PRESSURE).maxPressureCapacity(2 * SteamUtil.BASE_PRESSURE)
                        .allowOvercharge(true).build())
                .component(new MultiblockComponent(3,3,3,1,0,1))
        .component(new Blueprint());
        ...
        ROLLING_MILL = new CraftingComponent.Builder("rollingmill")
                ...
                .steam(2000, 10, SteamUtil.BASE_PRESSURE, 1.5f * SteamUtil.BASE_PRESSURE, true).create();

        FURNACE_MK1 = new CraftingComponent.Builder("furnacemk1").recipe(QBarRecipeHandler.FURNACE_UID, 1f)
                .inventory(3, 1, 1).steam(2000, 10, SteamUtil.BASE_PRESSURE, 1.5f * SteamUtil.BASE_PRESSURE, true)
                .create();

        FURNACE_MK2 = new CraftingComponent.Builder("furnacemk2").recipe(QBarRecipeHandler.FURNACE_UID, 1f)
                .inventory(3, 1, 1).steam(2000, 10, SteamUtil.BASE_PRESSURE, 1.5f * SteamUtil.BASE_PRESSURE, true)
                .create();

        ASSEMBLER = new CraftingComponent.Builder("assembler").recipe("", 1f).inventory(3, 1, 1)
                .steam(2000, 10, SteamUtil.BASE_PRESSURE, 1.5f * SteamUtil.BASE_PRESSURE, true).create();

        ORE_WASHER = new CraftingComponent.Builder("orewasher").recipe(QBarRecipeHandler.ORE_WASHER_UID, 1f)
        ...
        SAW_MILL = new CraftingComponent.Builder("sawmill")
                ...
                .steam(2000, 10, SteamUtil.BASE_PRESSURE, 1.5f * SteamUtil.BASE_PRESSURE, true)
                .create();*/
    }

    public static void loadMachines()
    {
        Gson gson = new GsonBuilder().registerTypeAdapter(MachineDescriptor.class, new MachineDescriptorTypeAdapter()).create();

        MachineDescriptor descriptor = null;
        final InputStream stream = Blueprints.class
                .getResourceAsStream("/assets/" + QBar.MODID + "/machines/smallminingdrill.hjson");
        try
        {
